...
def Sniff_Reseau_Win(where_to_sniff=None, N=1):
...
def Sniff_Clovis(where_to_sniff=None, N=1):
  if where_to_sniff:
    a=sniff(offline=where_to_sniff,prn=Dissection)
...
if __name__ == '__main__':
  ...
  if Trame_a_analyser==None:
    print "DATA sniffed = ",Sniff_Clovis(),"\n"
  else:

    data=Sniff_Clovis(Trame_a_analyser,N)
    #data.show()
    print "\nDATA = ", data,"\n"
  print"FIN SNIFF\n"
